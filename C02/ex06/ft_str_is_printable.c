...
/*
void ft_test(char *str, int expected)
{
...
int main(void)
{
    printf("--- Iniciando tests de ft_str_is_printable ---\n");

    // Casos que deben devolver 1 (Todo imprimible)
    ft_test("Hola Mundo!", 1);         // Incluye espacio y símbolos
    ft_test("1234567890", 1);          // Solo números
    ft_test("!@#$%^&*()_+~`|{}:<>?", 1); // Solo símbolos
    ft_test("", 1);                    //
    ...
    ft_test("Hola\tMundo", 0);         // Incluye tabulación
    ft_test("Hola\x01Mundo", 0);       // Incluye carácter de control ASCII 1
    ft_test("Hola\x7F Adios",  0);       // Incluye carácter DEL (ASCII 127)
